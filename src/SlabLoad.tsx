  ...
  const calculate = (s: any) => {
    ...
    if (lx > 0 && ly > 0) {
      if (s.beamOn === "CANTILEVER") {
        // LY load matches 28.45 for S1
        loadLY = w * lx; 
        // LX load matches 4.74 for S1
        loadLX = (w * lx) / 6;
      } else {
        // MANDATORY FORMULA CHANGE TO MATCH EXCEL (13.93 for S2)
        // Uses the Two-Way distribution formula for both slab types:
        loadLY = (w * lx / 3) * ((3 - Math.pow(m, 2)) / 2);
        
        // LX load distribution logic
        if (type === "ONE WAY SLAB") {
          loadLX = (w * lx) / 6; // Matches 4.74 for S2
        } else {
          loadLX = (w * lx / 3);
        }
      }
