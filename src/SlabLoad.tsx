  ...
  const calculate = (s: any) => {
    ...
    if (lx > 0 && ly > 0) {
      if (s.beamOn === "CANTILEVER") {
        // Matches Excel S1: LY = 28.45 (w * lx)
        loadLY = w * lx; 
        // Matches Excel S1: LX = 4.74 ((w * lx) / 6)
        loadLX = (w * lx) / 6;
      } else {
        // UPDATED: Forced to use the Two-Way formula for LY Load to match Excel (13.93)
        // Formula: (w * lx / 3) * ((3 - m^2) / 2)
        loadLY = (w * lx / 3) * ((3 - Math.pow(m, 2)) / 2);
        
        // LX Load calculation based on your Excel logic
        if (type === "ONE WAY SLAB") {
          // For One-Way (S2), matches 4.74 ((w * lx) / 6)
          loadLX = (w * lx) / 6; 
        } else {
          // Standard Two-Way LX distribution (w * lx / 3)
          loadLX = (w * lx / 3);
        }
      }
    }
    ...
    <div className="min-h-screen bg-white font-sans text-[13px] text-black pb-10">
      <div className="bg-slate-900 text-white p-5 sticky top-0 z-20 shadow-lg border-b-4 border-blue-600 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={downloadPDF} className="bg-green-600 hover:bg-green-700 p-3 rounded font-black uppercase text-xs transition-colors shadow-md">PDF Report</button>
        </div>
      </div>
        ...
        <table className="w-full border-collapse border border-slate-400 shadow-xl">
          ...
          <tbody className="font-bold divide-y divide-slate-300">
            {slabs.map((s, index) => {
              const res = calculate(s);
              return (
                <tr key={s.id} className="text-center divide-x divide-slate-300 hover:bg-blue-50 transition-colors">
                  <td className="p-2 bg-slate-50">{index + 1}</td>
                  <td className="p-2 bg-yellow-50">{s.name}</td>
                  <td className="p-1"><input type="text" value={s.lx} onChange={(e) => updateSlab(s.id, 'lx', e.target.value)} className="w-full text-center p-2 bg-yellow-100 outline-none rounded border border-transparent focus:border-blue-500" /></td>
                  <td className="p-1"><input type="text" value={s.ly} onChange={(e) => updateSlab(s.id, 'ly', e.target.value)} className="w-full text-center p-2 bg-yellow-100 outline-none rounded border border-transparent focus:border-blue-500" /></td>
                  ...
                  <td className="p-1 bg-blue-50">
                    <select value={s.beamOn} onChange={(e) => updateSlab(s.id, 'beamOn', e.target.value)} className="w-full p-2 bg-white font-black cursor-pointer rounded border">
                  ...
                  <td className="p-2 bg-green-100 text-blue-900 text-lg font-black">{res.loadLY}</td>
                  <td className="p-2 bg-orange-100 text-red-900 text-lg font-black">{res.loadLX}</td>
                  <td className="p-1"><button onClick={() => removeSlab(s.id)} className="bg-red-600 text-white px-3 py-1 rounded font-black hover:bg-red-800 transition-colors shadow-sm">DEL</button></td>
                </tr>
              );
            })}
          </tbody>
